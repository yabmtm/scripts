...
#infile = sorted(glob.glob("*mol2"))[0]
#fin = open(infile,'r')
#rawchunks = ['@'+s for s in fin.read().split('@')]
#rawchunks.pop(0) # this first chunk is blank
#fin.close()

#chunks = {}  # a dictionary with the @<TRIPOS> keyword as the key
...
#    chunks[keyword] = fields
#    chunkkeys.append(keyword)
...
class Mol2File:
    ...
    def extract_atoms(self):
        ...
        for i in range(len(self.chunks['ATOM'])): #self.chunks
            line = self.chunks['ATOM'][i] # self.chunks
            if len(line) > 8:
                fields = line.split()
...
#            print self.chunks['ATOM'][i]
    ...
    def edit_charge(self, name, charge):
        ...
        try:
            ...
            print(e, "Mismatching set of atoms between CappingAtoms and input." )
